    ...
    override fun postProcessBeanFactory(beanFactory: ConfigurableListableBeanFactory) {
        beanFactory.getBeanDefinition(beanName).let { beanDefinition ->
            Class.forName(beanDefinition.beanClassName).let { bean ->
                bean.interfaces.find { it.simpleName == interfaceName }?.declaredMethods
                    ?.find { it.isAnnotationPresent(PostConstruct::class.java) }.let { interfaceMethod ->
                        bean.declaredMethods.find { it.name == interfaceMethod?.name }
